<?php
    ...
    //consultas
    $pedido = $_POST['pedido_mesas'];
        // consulta ingreso de datos de la mesa a base datos    
            if(isset($_POST['pedido_mesas'])){
                if(strlen($_POST['pedido_mesero']) >= 1 && strlen($_POST['pedido_mesas']) >= 1 && strlen($_POST['pedido_estado']) >= 1){
                    $fecha = date("y/m/d/H/i/s");
                    $mesero = trim($_POST['pedido_mesero']);
                    $mesa = trim($_POST['pedido_mesas']);
                    $estado = trim($_POST['pedido_estado']);
                    
                    $consulta = "   INSERT INTO pedidos(pedido_fecha, pedido_mesero, pedido_mesa, pedido_estado) 
...
//consultas
    ...
    // lista de productos
        ...
        // ultimo codigo de recibo
            $query_productos2 = "   SELECT  
                                    codigo_recibo as cod,
            
                                    pedido_mesa as mesa
            
                                    FROM        pedidos AS PD
            
                                    WHERE codigo_recibo = (SELECT MAX(codigo_recibo) from pedidos)
                                    ";
        ...
        if(isset($_GET['pedido_mesa'])){
            ...
            $mesaactiva1 = $_GET['pedido_mesa'];

            
            $mesaactiva = $mysqli->query( " SELECT  
    ...
    // ultimo codigo de recibo
        $query_cant_prod = "    SELECT COUNT(detalle_producto) as total_cantidad,
                                ...
                                FROM pedido_detalle AS PD
                                INNER JOIN  pedidos AS PE ON PD.codigo_recibo_detalle=PE.codigo_recibo
                                WHERE pedido_mesa = 1 AND codigo_recibo_detalle = (SELECT MAX(codigo_recibo) from pedidos)";
                                                ...
                                                <div class="col col-6 col-sm-4 col-md-4 col-lg-4 col-xl-4" >
                                                    ...
                                                    <form method="POST" action="pedido_mesa1.php" class="m-0">
                                                       ...
                                                       <input type="hidden" name="detalle_cantidad" value="1"></input>
                                                        ...
                                                        <input type="hidden" name="detalle_estado" value="activo"></input>                                                        
